...
def calculate_normalized_fitness(population, age, score_list=None):
    # Scoring_function
    if age == 0:
        score = multi_scoring_functions(population, ['drd2', 'qed', 'sa'])
    else:
        score = score_list
    print('age: ', age)
...
for i in range(101):
    ...
    try:
        for generation in range(generations):
            ...
            every_pop.to_csv('./output/top3/'+str(i)+'_'+str(age)+'_every_top3.csv', index=False, header=False, mode='a')

            all_pop = pd.concat([pd.DataFrame(all_population), pd.DataFrame(all_population_score)], axis=1)
            all_pop.to_csv('./output/all_pop/' +str(i)+'_'+ str(age) + '_every_all.csv', index=False, header=False, mode='a')
